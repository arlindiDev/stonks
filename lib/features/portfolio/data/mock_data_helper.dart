...
class MockDataHelper {
  ...
  // 6M data - 27 points at weekly intervals (Jun 6 to Dec 6, 2025)
  static List<proto.ChartDataPoint> _create6MData() {
    final values = [
      // June 2025 (starting from pullback period, aligning with YTD/1Y)
      116000.0, 116400.0, 116800.0, 116000.0,
      // July 2025 (rally begins)
      115200.0, 114400.0, 115000.0, 116000.0, 117000.0, 118000.0,
      // August 2025 (continued rally)
      119000.0, 120000.0, 121000.0, 122000.0, 123000.0,
      // September 2025 (consolidation)
      124000.0, 126000.0, 126000.0, 125800.0,
      // October 2025 (mixed)
      125600.0, 126200.0, 126800.0, 127000.0, 126700.0,
      // November 2025 (recovery)
      126800.0, 127100.0,
      // December 2025 (current)
      127300.0, 127439.25
    ];
    final start = DateTime(2025, 6, 6, 9, 30);
    final initialValue = values.first;
    return List.generate(values.length, (i) {
      final value = values[i];
  ...
  // YTD data - 113 points at 3-day intervals (Jan 1 to Dec 6, 2025)
  static List<proto.ChartDataPoint> _createYTDData() {
    final values = [
      // January - February (uptrend)
      ...
      126000.0, 126200.0, 126100.0, 125900.0, 125700.0, 125500.0, 125400.0, 125600.0, 125800.0, 126000.0,
      // October (uptrend)
      126200.0, 126400.0, 126600.0, 126800.0, 127000.0, 127200.0, 127100.0, 126900.0, 126700.0, 126500.0,
      // November (volatility then recovery)
      126400.0, 126600.0, 126800.0, 127000.0, 127200.0, 127100.0, 126900.0, 127100.0, 127300.0, 127200.0,
      // December (current, strong finish)
      127100.0, 127300.0, 127439.25
    ];
    final start = DateTime(2025, 1, 1, 9, 30);
    final initialValue = values.first;
    return List.generate(values.length, (i) {
      final value = values[i];
  ...
  // 1Y data - 92 points at 4-day intervals (Dec 6, 2024 to Dec 6, 2025)
  static List<proto.ChartDataPoint> _create1YData() {
    final values = [
      // December 2024 (starting point, gradual increase into new year)
      100000.0, 100500.0, 101000.0, 101500.0, 102000.0, 102500.0, 103000.0, 104000.0,
      // January 2025 (aligning with YTD pattern)
      105000.0, 105400.0, 105800.0, 106200.0, 106600.0, 107000.0, 107400.0, 107800.0, 108200.0, 108600.0,
      109000.0, 109400.0, 109800.0, 110200.0,
      // February 2025
      110600.0, 111000.0, 111400.0, 111800.0, 112200.0, 112600.0,
      // March 2025 (correction)
      112300.0, 112000.0, 111700.0, 111400.0, 111100.0, 110800.0, 110500.0, 110200.0,
      // April 2025 (recovery starts)
      110000.0, 110400.0, 110800.0, 111200.0, 111600.0, 112000.0, 112400.0, 112800.0,
      // May 2025
      113200.0, 113600.0, 114000.0, 114400.0, 114800.0, 115200.0, 115600.0, 116000.0,
      // June 2025 (pullback)
      116400.0, 116800.0, 116800.0, 116000.0, 115200.0, 114600.0, 114000.0, 113800.0,
      // July 2025 (rally)
      114400.0, 115000.0, 115600.0, 116200.0, 116800.0, 117400.0, 118000.0, 118600.0,
      // August 2025
      119200.0, 119800.0, 120400.0, 121000.0, 121600.0, 122200.0, 122800.0, 123400.0,
      // September 2025 (consolidation)
      123800.0, 124200.0, 126000.0, 126200.0, 126100.0, 125900.0, 125700.0, 125600.0,
      // October 2025
      125800.0, 126000.0, 126200.0, 126600.0, 126800.0, 127000.0, 127000.0, 126800.0,
      // November 2025 (volatility then recovery)
      126600.0, 126800.0, 127000.0, 127100.0, 127000.0, 127100.0, 127200.0,
      // December 2025 (current)
      127300.0, 127439.25
    ];
    final start = DateTime(2024, 12, 6, 9, 30);
    final initialValue = values.first;
    return List.generate(values.length, (i) {
      final value = values[i];
